...
class BubbleStation: BubbleSubscriber{
    ...
    struct Constants{
        ...
        static let MAX_HEIGHT = 230.VAdapted
        static let MIN_HEIGHT = 80.VAdapted
        static let MIN_WIDTH_ODD = 80.VAdapted
        static let MIN_WIDTH_EVEN = 40.VAdapted
        static let VERTICAL_DISTANCE_BETWEEN_BUBBLES = 80
    ...
    func createAvaiableSpawnPosition(){
        ...
        for raw in 1..<4{
            let maxColumn = raw % 2 == 0 ? Constants.SPACE_TO_OCCUPY_EVEN:Constants.SPACE_TO_OCCUPY_ODD
            let rawHeight = Int(Constants.MAX_HEIGHT) - (raw - 1)*Constants.VERTICAL_DISTANCE_BETWEEN_BUBBLES
            for columnIndex in 0..<maxColumn{
                let minWidth = Int(raw % 2 == 1 ? Constants.MIN_WIDTH_EVEN : Constants.MIN_WIDTH_ODD)
